...
"""Dedicated V13.4 best-parameter memory."""
...
BEST_FILE_NAME = "best_parameters_V13_4.xlsx"
...
def save_best(
    ...
    *,
    ...
    if "v13_4_metadata" in workbook.sheetnames:
        del workbook["v13_4_metadata"]
    ws = workbook.create_sheet("v13_4_metadata")
    ws.append(list(metadata.keys()))
    ws.append(list(metadata.values()))
    workbook.save(output)

    return {"action": "saved_v13_4_best", "best_file": str(output), "backup_file": str(backup or ""), **metadata}
...
def restore_best(config_file: str | Path, results_dir: str | Path) -> dict[str, Any]:
    ...
    backup_dir = Path(results_dir) / "v13_4_parameter_backups"
    backup_dir.mkdir(parents=True, exist_ok=True)
    backup = backup_dir / f"{config_file.stem}_before_v13_4_restore_{datetime.now():%Y%m%d_%H%M%S}{config_file.suffix}"
    if config_file.exists():
        shutil.copy2(config_file, backup)
    shutil.copy2(source, config_file)
    return {"action": "restored_v13_4_best", "best_file": str(source), "config_backup_file": str(backup)}
...
def metadata(results_dir: str | Path) -> dict[str, Any]:
    ...
    if not source.exists():
        return {}
    try:
        df = pd.read_excel(source, sheet_name="v13_4_metadata")
